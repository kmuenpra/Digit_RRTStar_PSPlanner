...
def get_robot(cfg, sensor):
    robot = pypolo.robots.DiffDriveRobot(
        sensor=sensor,
        state=np.array([0.01, 0.01, 0.0]), #np.array([cfg.task_extent[1], cfg.task_extent[2], -np.pi]),
...
def get_planner(cfg, rng, start=None, goal=None, obstacles=None):
...
def information_gathering(robot, model, planner, samples_per_dt, final_goal, visualizer):
    ...
    while True:
        ...
        goal = planner.plan(model, robot_location=robot.state[:2], current_heading=robot.state[2])
        ...
        while robot.has_goals:
            ...
            robot.step_with_heading(current_heading=robot.state[2], num_targets = samples_per_dt)
...
def main(cfg: DictConfig) -> None:
    ...
    planner =  get_planner(cfg, rng, start=start, goal=goal, obstacles=obstacles)
    ...
    decision_epoch = 0
    start_time = time()
    while planner.distance(robot.state[:2], goal) > robot.goal_radius:#len(evaluator.y_train) < cfg.max_num_samples:
