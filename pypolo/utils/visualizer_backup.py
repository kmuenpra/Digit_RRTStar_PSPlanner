...
class Visualizer:
    ...
    def init_axes(self) -> None:
        ...
        self.fig, axes = plt.subplots(1, 4, figsize=(16, 6), height_ratios=[1]) #height_ratios=[2,1]
    ...
    def plot_PSP_traj(self, cfg, robot, planner, global_planner, localGridIndex, localGrid_info, end_flag=False):
        ...
        if len(all_wp_x) == len(body_x_axis):
            for i in range(len(body_x_axis)):
                ...
                self.axes[4].arrow(all_wp_x[i], all_wp_y[i],  *4*body_y_axis[i], head_width=0.001, head_length=0.001, fc='k', ec='k', alpha=0.1)
        self.axes[4].set_xlim(np.array(robot.history["apex_state"])[-1, 0] - 0.4, np.array(robot.history["apex_state"])[-1, 0] + 0.4)
        self.axes[4].set_ylim(np.array(robot.history["apex_state"])[-1, 1] - 0.4, np.array(robot.history["apex_state"])[-1, 1] + 0.4)
        self.axes[4].grid()
        self.axes[4].legend()
        ...
        self.apex1 = self.axes[1].plot(np.array(robot.history["foot_position"])[:,0], np.array(robot.history["foot_position"])[:,1], color="yellow", marker="*",markersize=2)
