...
class RrtStar:
    ...
    def planning(self):
        for k in range(self.iter_max):
            ...
            if math.hypot(node_new.x - self.s_goal.x, node_new.y - self.s_goal.y) <= 0.5:
                break
                    
        index = self.search_goal_parent()
        self.path = self.extract_path(self.vertex[index])
        self.plotting.animation(self.vertex, self.path, "rrt*, tota Iter. = " + str(k))
...
def main():
    ...
    rrt_star = RrtStar(x_start, x_goal, step_len=0.5, goal_sample_rate=0.10, search_radius=0.6, iter_max=10000)
    rrt_star.planning()
